...
const ManufacturerDetails = () => {
  ...
  const handleDownloadPDF = async () => {
    ...
    doc.setFontSize(22);
    doc.text("Manufacturer Report", pageWidth / 2, 20, { align: "center" });
    ...
    if (manufacturer.image) {
      try {
        const response = await fetch(manufacturer.image);
        if (response.ok) {
          ...
          doc.addImage(imgData, "JPEG", margin + 20, 35, contentWidth - 40, 50);
          imgHeight = 60;
        }
      } catch (err) {
    ...
    const addField = (label, value) => {
      ...
      doc.setFont("helvetica", "normal");
      const splitValue = doc.splitTextToSize(String(value), contentWidth - labelWidth - 5);
      doc.text(splitValue, margin + labelWidth, yPos);
      yPos += Math.max(8, splitValue.length * 6);
    };

    // ✳️ General Information
    addSection("General Information");
    addField("Name", manufacturer.name);
    addField("Country", manufacturer.country);
    addField("Website", manufacturer.website);
    addField("Has Agent", manufacturer.has_agent ? "Yes" : "No");

    // وصف منسق
    doc.setFont("helvetica", "bold");
    doc.text("Description:", margin, yPos);
    doc.setFont("helvetica", "normal");
    const desc = manufacturer.description || "No description";
    const splitDesc = doc.splitTextToSize(desc, contentWidth - 10);
    doc.text(splitDesc, margin, yPos + 6);
    yPos += splitDesc.length * 6 + 8;

    // ✳️ Contact Information
    addSection("Contact Information");
    addField("Contact Name", manufacturer.contact_name);
    addField("Position", manufacturer.position);
    addField("Email", manufacturer.email);
    addField("Phone", manufacturer.mobile);
    addField("Telephone", manufacturer.telephone);
    addField("Address", manufacturer.address);

    // ✳️ Distributors
    addSection("Distributors");
    if (Array.isArray(manufacturer.distributors) && manufacturer.distributors.length > 0) {
      const body = manufacturer.distributors.map(d => [
        d.name || "—",
        d.contact_name || "—",
        d.email || "—",
        d.phone || "—",
      ...
      autoTable(doc, {
        startY: yPos,
        head: [["Name", "Contact Name", "Email", "Phone"]],
        body,
      ...
      doc.setFont("helvetica", "normal");
      doc.text("No distributors linked to this manufacturer.", margin, yPos);
    }

    // 💾 الحفظ
    const safeName = (manufacturer.name || "Manufacturer").replace(/[^a-z0-9\s]/gi, "_").trim();
    doc.save(`Manufacturer_Report_${safeName}.pdf`);
  };
